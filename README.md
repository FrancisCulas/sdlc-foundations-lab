...
Adaptability & Time-to-Market
	Less adaptable because requirements are mostly fixed, and releases usually take longer.	More adaptable and allows faster releases through short development sprints.

Regulatory & Compliance Risk Handling
	Strong for regulatory compliance because it follows a structured process with detailed documentation.	Can handle compliance, but requires proper documentation and compliance checks during development.
...
Answer: I recommend a hybrid approach using Agile (Scrum) with Waterfall practices for regulatory requirements. Scrum allows the team to develop and release the payment feature quickly, while Waterfall practices can provide the documentation and compliance checks required by the government agency.
...
Answer: The gap occurred because developers tested the code manually on their own laptops instead of using an automated testing process shared with the Operations team. This caused problems to reach production because the code was not tested in an environment similar to production.
...
Answer: Feature branches allow developers to work on their own features without directly changing the Main branch. The Develop branch is used to combine and test the different features before they are merged into Main, which helps prevent developers from overwriting each other's code.
